...
1. Run `Start-Sleep -Seconds {interval}` in the terminal using **sync mode with a timeout of `{interval × 1000 + 5000}` ms** so the agent actually blocks until the sleep completes. Do NOT use async mode — you must wait for the full interval before proceeding.
...
5. If no reply → log `Poll attempt {N} of {max}: no reply yet` and repeat from step 1
